...
if "base_name_key" not in st.session_state:
    st.session_state.base_name_key = 0
if "randomize_names" not in st.session_state:
    st.session_state.randomize_names = False
...
def create_github_blob(session, token, repo, content_bytes):
    ...
    try:
        ...
        if res.status_code == 201:
            ...
            return {
                "success": False,
                "error": res.json().get("message", "Unknown error"),
            }
    except Exception as e:
        return {"success": False, "error": str(e)}


def upload_batch_to_github(
    token, repo, branch, folder, file_infos, commit_message, progress_callback=None
):
    ...
    try:
        ...
        if ref_res.status_code != 200:
            return {
                "success": False,
                "error": f"Failed to get branch reference: {ref_res.json().get('message', 'Unknown error')}",
            }
        ...
        if commit_res.status_code != 200:
            return {
                "success": False,
                "error": f"Failed to get base commit: {commit_res.json().get('message', 'Unknown error')}",
            }
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            futures = {
                executor.submit(
                    create_github_blob, session, token, repo, img["content"]
                ): img
        ...
        tree_data = {"base_tree": base_tree_sha, "tree": tree_nodes}
        tree_res = session.post(tree_url, headers=headers, json=tree_data, timeout=30)
        if tree_res.status_code != 201:
            return {
                "success": False,
                "error": f"Failed to create new tree: {tree_res.json().get('message', 'Unknown error')}",
            }
        ...
        commit_post_res = session.post(
            commit_post_url, headers=headers, json=commit_data, timeout=30
        )
        if commit_post_res.status_code != 201:
            return {
                "success": False,
                "error": f"Failed to create commit: {commit_post_res.json().get('message', 'Unknown error')}",
            }
        ...
        ref_update_data = {"sha": new_commit_sha, "force": False}
        ref_update_res = session.patch(
            ref_update_url, headers=headers, json=ref_update_data, timeout=30
        )
        if ref_update_res.status_code != 200:
            return {
                "success": False,
                "error": f"Failed to update reference: {ref_update_res.json().get('message', 'Unknown error')}",
            }
...
with st.sidebar:
    ...
    gh_token = st.text_input(
        "GitHub PAT", type="password", help="Personal Access Token with 'repo' scope."
    )
    gh_repo = st.text_input(
        "Repository Path", placeholder="username/repo-name", help="Format: owner/repo"
    )
    ...
    base_name = st.text_input(
        ...
        help="Files will be named base-1.webp, base-2.webp, etc.",
        key=f"base_name_{st.session_state.base_name_key}",
    )
    st.session_state.randomize_names = st.toggle(
        "🎲 Randomize File Names",
        value=st.session_state.randomize_names,
        help="Generates unique random filenames (e.g. img_a3f8b2.webp) instead of sequential names.",
    )
...
st.markdown(
    '<div class="main-header">ImageSync Public <span>by Shady</span></div>',
    unsafe_allow_html=True,
)
st.markdown(
    '<div class="sub-header">Professional Image Optimization & GitHub Hosting</div>',
    unsafe_allow_html=True,
)
...
if uploaded_files:
    if not (gh_token and gh_repo):
        st.warning(
            "⚠️ Please provide your GitHub Token and Repository Path in the sidebar to proceed."
        )
    else:
        ...
        with st.container(border=True):
            ...
            # File list preview
            with st.expander(
                f"📁 Selected Files List ({len(uploaded_files)})", expanded=True
            ):
            ...
            with col_clear:
                if st.button("🧹 Clear All Files", use_container_width=True):
                    ...
                    st.session_state.base_name_key += 1
                    st.rerun()
            with col_deploy:
        ...
        if deploy_btn:
            ...
            for i, f in enumerate(uploaded_files):
                if st.session_state.randomize_names:
                    rand_suffix = "".join(
                        random.choices(string.ascii_lowercase + string.digits, k=6)
                    )
                    processed_names.append(f"img_{rand_suffix}.webp")
                elif base_name:
            ...
            with concurrent.futures.ThreadPoolExecutor() as executor:
                futures = [
                    executor.submit(optimize_image, f, n, quality)
                    for f, n in zip(uploaded_files, processed_names)
                ]
                for i, future in enumerate(concurrent.futures.as_completed(futures)):
            ...
            if valid_images:
                ...
                def update_progress(current, total, name):
                    status.markdown(
                        f"☁️ **Step 2/2: Uploaded {name} ({current}/{total})...**"
                    )
                ...
                if res["success"]:
                    ...
                    final_results = [
                        {"name": img["name"], "error": res["error"], "success": False}
